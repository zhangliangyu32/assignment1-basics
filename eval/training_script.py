...
# config["note"] = "post_norm ablation"
# config["learning_rate"] = 1e-3
# config["ablation_mode"] = "post_norm"
# model = TransformerLMwithAblation(vocab_size=10000, context_length=config["context_len"], num_layers=4, d_model=512, num_heads=16, d_ff=1344, rope_theta=10000, ablation_mode=config["ablation_mode"], device=config["device"])
# train(x_train, x_val, model, config)

# config["note"] = "no_RoPE ablation"
# config["ablation_mode"] = "no_RoPE"
# model = TransformerLMwithAblation(vocab_size=10000, context_length=config["context_len"], num_layers=4, d_model=512, num_heads=16, d_ff=1344, rope_theta=10000, ablation_mode=config["ablation_mode"], device=config["device"])
# train(x_train, x_val, model, config)
import torch
config["note"] = "owt with parameters tuned"
model = TransformerLM(vocab_size=32000, context_length=config["context_len"], num_layers=6, d_model=512, num_heads=16, d_ff=1344, rope_theta=10000, device=config["device"])
config["lr_scheduler_cosine"] = True
config["weight_decay"] = 0.001
config["max_iterations"] = 80000
config["cosine_end"] = 80000
config["warmup_end"] = 4000
config["batch_size"] = 64
config["train_data_path"] = "/root/autodl-tmp/data/owt_train.npy"
config["val_data_path"] = "/root/autodl-tmp/data/owt_valid.npy"
